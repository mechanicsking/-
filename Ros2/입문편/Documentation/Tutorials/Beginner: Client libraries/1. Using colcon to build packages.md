...
## Background
colcon은 ros2에서 제공되는 공식 빌드 도구로 기존의 catkin_make, catkin_make_isolated, catkin_tools 과 ament_tools 등 여러 빌드 도구의 발전형이다. colcon의 디자인에 대한 정보를 알고 싶다면 https://design.ros2.org/articles/build_tool.html 
 의 문서를 확인하면 된다. 소스코드는 https://github.com/colcon 에서 확인할 수 있다.       
...
colcon은 REP 149에서 정의된 package.xml 사양을 사용한다(형식 2도 지원됨). colcon은 여러 빌드 방식을 지원하는데, 권장되는 빌드 방식은 ament_cmake와 ament_python이다. 또한 순수 cmake 패키지도 지원한다.      
ament_python 빌드의 예는 ament_index_python 패키지로, 여기서 setup.py가 빌드의 주요 진입점 역할을 한다. demo_nodes_cpp 같은 패키지는 ament_cmake 빌드 타입을 사용하며 CMake를 빌드 도구로 활용한다.         
편리하게도, ros2 pkg create 도구를 사용해서 템플릿 기반으로 새 패키지를 만들 수 있다. 패키지 생성과 ros2 pkg create 사용법은 앞으로의 튜토리얼에서 자세히 다룰 예정이다.      
참고!- catkin 사용자에게는 이 도구가 catkin_create_package와 동일한 역할을 한다.     
## 3. Setup colcon_cd
colcon_cd 명령어는 터미널에서 빠르게 워크스페이스 내 특정 패키지 폴더로 이동할 수 있게 해준다. 예를 들어, colcon_cd some_ros_package를 실행하면 내 워크스페이스의 ~/ros2_ws/src/some_ros_package 폴더로 바로 이동한다.이 기능을 사용하려면아래 두 줄을 내 쉘 시작 스크립트(ex: ~/.bashrc)에 추가하여 설정해야 한다. 
```
$echo "source /usr/share/colcon_cd/function/colcon_cd.sh" >> ~/.bashrc
$echo "export _colcon_cd_root=/opt/ros/humble/" >> ~/.bashrc
```
이렇게 하면 터미널을 열 때마다 colcon_cd 명령이 자동으로 활성화된다. 설치 환경이나 워크스페이스 위치에 따라 위 경로는 다를 수 있으므로,자세한 내용은 공식 문서를 참고하는 게 좋다.(https://colcon.readthedocs.io/en/released/user/installation.html#quick-directory-changes) 만약 이 설정을 해제하려면,쉘 시작 스크립트에서 위 두 명령어를 삭제하면 된다. 
## 4. Setup colcon tab completion
colcon 명령어는 baxh나 bash와 비슷한 쉘(터미널 환경)에서 명령어 자동 완성 기능을 지워낳나. 이 자동 완성기능을 사용하려면, colcon-argcomplete라는 별도의 패키지를 설치해야 한다. 또한 이 기능이 정상 작동하도록 하기 위해서는 추가적인 설정 작업(setup)이 필요할 수 있다. (https://colcon.readthedocs.io/en/released/user/installation.html#enable-completion) 
## 5. Tips
* 만약 특정 패키지를 빌드하고 싶지 않으면, 그 패키지 폴더 안에 빈 파일 COLCON_IGNORE를 만들어 놓으면, colcon이 그 패키지를 무시(인덱싱하지 않음)한다.
* CMake 패키지에서 테스트 구성을 하지 않고 빌드하려면, 빌드 시 다음 옵션을 사용할 수 있습니다: --cmake-args -DBUILD_TESTING=0
* 특정 패키지에서 단일 테스트만 실행하고 싶으면 다음 명령어를 사용합니다:
  ```
  $colcon test --packages-select YOUR_PKG_NAME --ctest-args -R YOUR_TEST_IN_PKG
  ```

## 5. Setup colcon misins
다양한 명령줄 옵션들은 일일이 쓰기 번거롭고 기억하기 어려울 수 있다. 예를 들어, CMake 빌드 옵션을 디버그 모드로 바꾸려면 보통 이렇게 쓴다:
```
$colcon build --cmake-args -DCMAKE_BUILD_TYPE=Debug
```
이처럼 자주 사용하는 명령줄 옵션을 더 쉽게 쓰게 하기 위해, colcon에서는 “mixin”이라는 단축키 기능을 제공한다. 기본 colcon mixin을 설치하려면 아래 명령어를 실행하면 된다:
```
$colcon mixin add default https://raw.githubusercontent.com/colcon/colcon-mixin-repository/master/index.yaml
$colcon mixin update default
```
오류가 뜬다면 colcon-mixin 패키지가 설치되어 잇는지 확인하라
```
$pip install colcon-mixin
```
이제 debug mixin을 이용해보자. 
```
$colcon build --mixin debug
```
자세한 내용은 (https://github.com/colcon/colcon-mixin-repository) 를 참고하면된다. 
