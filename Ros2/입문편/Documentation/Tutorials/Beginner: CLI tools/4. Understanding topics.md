...
```
### 8. ros2 topic hz
데이터가 publishded되는 속도를 다음의 명령어를 통해 알 수 있다.    
```
$ros2 topic hz /turtle1/pose
average rate: 59.354
  min: 0.005s max: 0.027s std dev: 0.00284s window: 58
```
이 명령어는 /turtlesim 노드가 pose 토픽에 데이터를 publishing 하는 속도를 반환하게 된다..      
ros2 topic pub --rate1 명령어를 사용해 turtle1/cmd_vel의 속도를 일정한 1Hz로 설정했다는 것을 기억하라. 만약 위 명령어를 tutle1/pose 대신 turtle1/cmd_vel로 실행하면 해당 속도를 반영한 평균이 표시된다.        
 ros2 topic pub /turtle1/cmd_vel geometry_msgs/msg/Twist "{linear: {x: 2.0, y: 0.0, z: 0.0}, angular: {x: 0.0, y: 0.0, z: 1.8}}"를 실행한 후 이를 확인하면 1Hz 속도로 고정되어있음을 확인할 수 있다.    
<img width="1378" height="909" alt="image" src="https://github.com/user-attachments/assets/a90fad87-6036-4c7e-946a-fdb6d63ce432" />      

주의!-ros2 topic hz 명령이 메시지를 subscription하는 입장에서 받은 메시지를 기준으로 주기를 측정하기 때문에, 실제 발행 주기와 약간 차이가 있을 수 있고, 시스템 자원이나 QoS(품질 서비스) 설정 등에 영향을 받을 수 있다는 점이다.    
### 9. ros2 topic bw
토픽에 사용되는 bandwidth(대역폭)은 다음의 명령어를 통해 볼 수 있다. (대역폭이란? 특정 토픽을 통해 메시지가 송수신되는 데이터의 양,즉 한 토픽에서 초당 얼마나 많은 데이터가 오가는지를 측정하는 값)     
```
...
```
이 명령어는 /turtle1/pose 토픽에 대해 실제로 수신되는 데이터를 기준으로 토픽의 대역폭 사용챵(초당 몇 바이트가 오가는지)과 발행되는 메시지 개수를 보여준다.    
주의!-이 대역폭 측정치는 해당 명령어가 내부적으로 만든 subscriber가 실제로 받은 데이터를 바탕으로 계산한 값이라는 점이다. 즉 이 대역폭 값은 토픽을 발행하는 노드 publisher의 실제 대역폭과는 다를 수 있고, 사용중인 컴퓨터의 성능이나 네트워크 상황(플랫폼 자원), 메시지 전송 품질 설정등에 따라 영향을 받아 달라질 수 있다. 따라서, ros2 topic bw 명령어로 보는 대역폭은 "발행자가 보내는 정확한 데이터 양"이라기보다는 "구독자가 실제로 받은 데이터 양"에 가까운 값이며, 환경에 따라 어느 정도 오차가 있을 수 있다.      
### 10. ros2 topic find
주어진 타입의 사용가능한 토픽의 목록을 나열하려면 다음의 명령어를 사용한다.    
```
$ros2 topic find <topic_type>
```
cmd_vel 토픽은 다음의 토픽을 갖는다는 것을 기억하자.    
```
geometry_msgs/msg/Twist
```
find 명령어를 사용하면 메시지 유형이 주어졌을 때 사용할 수 있는 주제를 출력한다.   
```
$ros2 topic find geometry_msgs/msg/Twist
/turtle1/cmd_vel
```
### 11. Clean up
이 시점에서 많은 노드들이 작동중이다. 이것을 멈추기 위해서는 각 터미널 마다 ctrl+c를 누르면 된다는 것을 잊지 마라.    
## Summary
노드는 토픽에 정보를 publish하므로 다른 노드들도 해당 정보를 subscribe할 수 있고 access할 수 있다. 이범 튜토리얼에서는 rqt_graph와 명령줄 도구를 사용하여 토픽에 대한 여러 노드 간의 연결을 살펴보았다. 이제 ros2에 데이터가 어떻게 이동하는지 알 수 있을 것이다.   
